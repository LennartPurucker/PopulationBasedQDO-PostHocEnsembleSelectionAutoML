...
class DiscreteWeightSpaceEmitter(EmitterBase):
    ...
    def __init__(self, archive, n_base_models: int, start_weight_vectors: List[Tuple[np.array, int]], batch_size: int,
                 starting_step_size: int = 1, elite_selection_method: str = "deterministic",
                 crossover: Optional[str] = None, crossover_probability: Union[float, Callable] = 1.,
                 crossover_probability_dynamic: bool = False,
                 mutation_probability_after_crossover: Union[float, Callable] = 1.,
                 mutation_probability_after_crossover_dynamic: bool = False,
                 negative_steps: bool = False, shared_seen_resources: Optional[Tuple[set, dict]] = None,
                 weight_random_elite_selection: bool = False, weight_random_step_selection: bool = False,
        ...
        if elite_selection_method not in self.allowed_elite_selection_methods:
            raise ValueError(
                "Unknown elite_selection_method! Allowed are: {}. Got: {}.".format(self.allowed_elite_selection_methods,
        ...
        if (not isinstance(mutation_probability_after_crossover, float)) and (
            ...
            raise ValueError("mutation_probability_after_crossover must be float or  Callable",
        ...
        if isinstance(mutation_probability_after_crossover, float):
            self._mutation_probability_after_crossover = lambda x: mutation_probability_after_crossover
        else:
        ...
        if (not isinstance(crossover_probability, float)) and (not isinstance(crossover_probability, Callable)):
            raise ValueError("crossover_probability must be float or  Callable",
        ...
        if isinstance(crossover_probability, float) and ((crossover_probability <= 0) or (crossover_probability > 1)):
            raise ValueError("for crossover_probability it must hold that hold that 0 < crossover_probability <=1",
        ...
        if mutation_probability_after_crossover_dynamic and (
            ...
            raise ValueError("_mutation_probability_after_crossover_dynamic can only be " +
    ...
    def ask(self):
        ...
        self._elite_origin_reject_counter = {k: self._tmp_elite_origin_reject_counter[k]
                                                + self._elite_origin_reject_counter[k] for k in
                                             self._elite_origin_reject_counter.keys()}
    ...
    def tell(self, solutions, objective_values, behavior_values, metadata=None):
        ...
        loop_data = zip(solutions, objective_values, behavior_values, self.last_ask_sample_sizes,
                        self.last_solutions_origins, self.last_elite_origins)
        ...
        self._rejections_over_time.append((self._tmp_crossover_rejects,
        ...
        for rejcs in self._rejections_over_time[N:]:
            ...
            tmp_val_2 = {k: rejcs[-1][k] + self._rejections_counts[-1][k] for k in
                         self._rejections_counts[-1].keys()}
    ...
    def _update_crossover_probability(self):
        ...
        if self._crossover_probability_dynamic and (self._origin_performance[SolutionOrigins.crossover][2] > 2) and \
                (self._origin_performance[SolutionOrigins.no_crossover][2] > 2):
            ...
            if self.dynamic_updates_consider_rejections:
                reject_ratio_crossover = self._origin_performance[SolutionOrigins.crossover][-1] / (
                        self._origin_performance[SolutionOrigins.crossover][-1] + self._rejections_counts[0] +
            ...
            # Update
            cr_prb = _avg_obj_update(self._origin_performance[SolutionOrigins.crossover],
    ...
    def _update_mutation_probability_after_crossover(self):
        if self._mutation_probability_after_crossover_dynamic and (
                self._origin_performance[SolutionOrigins.co_mutation][2] > 2) and \
                (self._origin_performance[SolutionOrigins.co_no_mutation][2] > 2):
            ...
            cr_prb = _avg_obj_update(self._origin_performance[SolutionOrigins.co_mutation],
    ...
    def _update_random_elite_selection_probability(self):
        if (self._elite_selection_method == "combined_dynamic") \
                and (self._origin_performance[EliteOrigins.stochastic][2] > 2) and \
                (self._origin_performance[EliteOrigins.deterministic][2] > 2):

            # Consider reject rate in performance assessment
            if self.dynamic_updates_consider_rejections:
                reject_ratio_stochastic = self._origin_performance[EliteOrigins.stochastic][-1] / (
                        self._origin_performance[EliteOrigins.stochastic][-1] + self._rejections_counts[-1][
                    EliteOrigins.stochastic])

                reject_ratio_deterministic = self._origin_performance[EliteOrigins.deterministic][-1] / (
                        self._origin_performance[EliteOrigins.deterministic][-1] + self._rejections_counts[-1][
                    EliteOrigins.deterministic])
            ...
            rng_prb = _avg_obj_update(self._origin_performance[EliteOrigins.stochastic],
    ...
    def _update_base_model_sample_weights(self, solutions, objective_values):
        if self._weight_random_step_selection:
            ...
            avg_perf_per_bm = np.divide((solutions * objective_values.reshape(-1, 1)).sum(axis=0),
    ...
    def _get_mutated_children(self, use_crossover):
        ...
        if do_crossover:
            ...
            for c_elite_float_w, c_sample_size in children:
                ...
                do_mutation_after_crossover = self._rng.random() < (self.mutation_probability_after_crossover
    ...
    def _edge_case_select_next_elite(self, n_elites=1):
        ...
        if n_elites > len(self._original_start_weight_vectors):
            ...
            while len(elites) < n_elites:
                ...
                sel_idx = self._rng.choice(len(self._original_start_weight_vectors),
    ...
    def _tournament_selection(self, n_elites):
        ...
        if not elites:
            ...
            elites = np.array([(elite_float_w, sample_size) for elite_float_w, _, _, _, sample_size in elites],
                              dtype="object")
    ...
    def _step_for_weights(self, discrete_weight_vector):
        ...
        if self._weight_random_step_selection:
            selected_step = tuple(self._rng.choice(self._remaining_steps_for_hash[dwv_hash],
                                                   p=softmax([self._base_model_sample_weights[idx]
...
def _avg_obj_update(origin_performance_stats_1, origin_performance_stats_2):
    # Get avg obj score
    avg_rng_perf = origin_performance_stats_1[0] / \
                   origin_performance_stats_1[2]
    avg_deter_perf = origin_performance_stats_2[0] / \
                     origin_performance_stats_2[2]
